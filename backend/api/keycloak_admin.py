...
confidentiel compagnon (``lab-admin-admin``, flux client_credentials). Il sert à
créer des utilisateurs (écrits dans le LDAP car la fédération est en
``editMode=WRITABLE``), poser un mot de passe et gérer l'appartenance aux
groupes (écrite dans le LDAP via le group mapper ``LDAP_ONLY``).

Porté de ``restauration/backend/api/keycloak_admin.py`` : même mécanique de
token/requête, généralisée à *tous* les groupes du realm (restauration se
limitait à son propre sous-ensemble applicatif).

La portée fonctionnelle (qui peut appeler, quels rôles) est bornée **côté
vues** — ce module reste un simple transport.
"""
...
def _token() -> str:
    ...
    if not client_id or not secret:
        raise KeycloakAdminError(
            ...
            "Lancez setup2.sh lab-admin (provisionne le client lab-admin-admin)."
        )
...
def all_groups() -> list[dict]:
    """Tous les groupes du realm : [{id, name}]."""
...
def find_user_by_username(username: str) -> dict | None:
    users = _req('GET', '/users', params={'username': username, 'exact': 'true'}).json()
...
def set_password(user_id: str, password: str, *, temporary: bool = False) -> None:
    _req(
